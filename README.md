...
3. If you have `pipenv (https://pypi.org/project/pipenv/)` installed, run `pipenv install` to install all required dependencies from `Pipfile`. Alternatively, run `pip3 install -r requirements.txt` to install all required dependencies from the `requirements.txt`.
